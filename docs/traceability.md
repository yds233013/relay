...
| GV-05 | Tested | `backend/src/relay/audit/instrumentation.py` fails any transaction that mutated a governed table without inserting an audit event; the autouse fixture in `tests/integration/conftest.py` runs it over the whole integration suite, `tests/integration/test_audit_instrumentation.py` proves it can fail (an unaudited insert, update and bulk update) and that exempt operational writes pass, `backend/tests/unit/test_audit_classification.py` keeps the governed/exempt classification covering every mapped table, `tests/integration/test_persistence.py` covers rollback atomicity. The guarantee is per transaction ("something was audited"), not per row ("the right event for the right row"); the listener sees the ORM unit of work, so `copy_rows` (raw `COPY`) writes are invisible and all of those tables are classified exempt. 25 tables are governed, 16 exempt with a reason each (job queue, append-only ingestion, per-run recompute output, and the investigation `running` transition, which the worker writes between the audited `investigation.requested` and `investigation.finished`). One classification is a judgement call worth naming: `investigations` is exempt because the worker commits its `queued → running` transition in its own transaction without an event, bracketed by the audited `investigation.requested` and `investigation.finished`. That intermediate transition is genuinely unaudited; auditing it would be a production behaviour change, out of scope for a presentation pass |
